...
public class PlayCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event) 
    {
        if(event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty())
        {
            ...
            builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" <song title>` - اجرای اولین کسشری که تو یوتیوب پیدا کردیم");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" <URL>` - اجرای پلی لیست ، فیلم یوتیوب یا استریم");
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist)
        {
            ...
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" این کسشر (**"+track.getInfo().title+"**) طولانی تر از حد مجازه کسخل:  `"
                        +FormatUtil.formatTime(track.getDuration())+"` > `"+FormatUtil.formatTime(bot.getConfig().getMaxSeconds()*1000)+"`")).queue();
                return;
            }
            AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
            int pos = handler.addTrack(new QueuedTrack(track, event.getAuthor()))+1;
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" اضافه شد **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"به اول":" لیست در این مکان اضافه شد "+pos));
