...
public class SearchCmd extends MusicCommand 
{
    ...
    public void doCommand(CommandEvent event) 
    {
        ...
        event.reply(searchingEmoji+" درحال جستجو... `["+event.getArgs()+"]`",
    ...
    private class ResultHandler implements AudioLoadResultHandler 
    {
        ...
        public void trackLoaded(AudioTrack track)
        {
            ...
            m.editMessage(FormatUtil.filter(event.getClient().getSuccess()+" Added **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0 ? "داره میاااااد"
        ...
        public void playlistLoaded(AudioPlaylist playlist)
        {
            builder.setColor(event.getSelfMember().getColor())
                    .setText(FormatUtil.filter(event.getClient().getSuccess()+" نتایج جست و جو `"+event.getArgs()+"`:"))
        ...
        {
            m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" چنین کسشری نیست `"+event.getArgs()+"`.")).queue();
        }
