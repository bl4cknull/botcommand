...
public class AutoplaylistCmd extends OwnerCommand
{
    ...
    public AutoplaylistCmd(Bot bot)
    {
        ...
        this.help = "یه پلی لیست تخمی برای سرور تنظیم کن";
    ...
    public void execute(CommandEvent event) 
    {
        ...
        {
            event.reply(event.getClient().getError()+" یه اسم بزار برا این کیرخر");
            return;
        }
        ...
        {
            event.reply(event.getClient().getError()+" نمیتونم اینو پیدا کنم `"+pname+".txt`!");
        }
        else
        {
            Settings settings = event.getClient().getSettingsFor(event.getGuild());
