...
public class PlaylistCmd extends OwnerCommand 
{
    ...
    {
        StringBuilder builder = new StringBuilder(event.getClient().getWarning()+" دستورات اجرای پلی لیست:\n");
    ...
    public class MakelistCmd extends OwnerCommand 
    {
        ...
        {
            this.name = "درست کردن";
            this.aliases = new String[]{"ساختن"};
            this.help = "درست کردن یه پلی لیست";
            this.arguments = "<name>";
            this.guildOnly = false;
        }
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(bot.getPlaylistLoader().getPlaylist(pname)==null)
            {
                try
                {
                    ...
                    event.reply(event.getClient().getSuccess()+" ماشالا این پلی لیستو ساختی `"+pname+"`!");
                }
                catch(IOException e)
                {
                    event.reply(event.getClient().getError()+" من نتونستم پلی لیست کیریتو بسازم:  "+e.getLocalizedMessage());
                }
            }
            else
                event.reply(event.getClient().getError()+" پلی لیست `"+pname+"` وجود داره (ریدی)");
        }
    }
    ...
    public class DeletelistCmd extends OwnerCommand 
    {
        ...
        {
            this.name = "پاک کردن";
            this.aliases = new String[]{"پاک کردن"};
            this.help = "پاک کردن پلی لیست مشابه";
            this.arguments = "<name>";
            this.guildOnly = false;
        }
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            else
            {
                try
                {
                    ...
                    event.reply(event.getClient().getSuccess()+" با موفقیت گاییده شد `"+pname+"`!");
                }
                catch(IOException e)
                {
                    event.reply(event.getClient().getError()+" من نمیتونم این پلی لیست کیریو پاک کنم: "+e.getLocalizedMessage());
                }
            }
        }
    }
    ...
    public class AppendlistCmd extends OwnerCommand 
    {
        ...
        {
            this.name = "اضافه شد";
            this.aliases = new String[]{"افزوده شد"};
            this.help = "موزیک به پلی لیست اضافه شد";
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(parts.length<2)
            {
                event.reply(event.getClient().getError()+" یدونه پلی لیست و ادرس اضافه کن!");
                return;
            }
