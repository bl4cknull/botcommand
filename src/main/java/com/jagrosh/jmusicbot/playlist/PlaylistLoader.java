                ...
                manager.loadItemOrdered(name, items.get(i), new AudioLoadResultHandler() 
                {
                    ...
                    public void trackLoaded(AudioTrack at) 
                    {
                        if(config.isTooLong(at))
                            errors.add(new PlaylistLoadError(index, items.get(index), "این ترک طولانی تر از حد مجاز است"));
                        else
                        {
                            at.setUserData(0L);
                            tracks.add(at);
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "مورد مشابه پیدا نشد"));
                        done();
                    }
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "پیدا کردن این ترک موفق نبود:  "+fe.getLocalizedMessage()+" کسخل دقت کن! "));
                        done();
                    }
                });
            }
